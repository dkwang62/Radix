...
| UI-02 | Addressed. Full restore validates both embedded databases, durably stores and verifies a complete pre-restore document, then records intent before mutation. Caught failures and startup both replay rollback; normal UI remains blocked until recovery succeeds. | The isolated Mac probe passed five interrupted incoming writes and four interrupted rollback writes. Physical-device interruption, storage-exhaustion and shipping-UI timing remain open. |
...
**Remediation status (2026-09-09):** Addressed with rollback journaling. A verified portable snapshot containing both SQLite databases, page images and portable preferences is flushed before durable intent. Sentence, phrase, image and preference persistence must complete before intent is retired. A caught failure immediately replays rollback; process interruption leaves intent for startup recovery before caches, extracted pages or normal UI load. Recovery is idempotent, blocks ordinary data work, exposes only Retry on failure, and preserves the prior rollback contract rather than finishing the incoming restore. The focused suite covers journal ordering, corrupt/missing artifacts and retry after interruption. The separate process probe passed five incoming-store SIGKILL boundaries and four rollback SIGKILL boundaries against synthetic store files. Actual shipping-app interruption on a disposable physical-device library, disk-full behavior and large-backup timing remain unverified.
