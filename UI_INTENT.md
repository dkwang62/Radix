...
title, thumbnail, small artifact indicators, and expand affordance visible.
Tapping the row expands that page in place; the expanded state offers the
`Actions` menu for viewing or saving translation, AI OCR review, phrase
extraction, quiz generation, sentence extraction, page conversation generation,
and deleting the page with its impact summary. Show a compact `Phrases` artifact
