...
enum DataExportDatabaseBuilder {
    static func makeMergedDictionaryDatabase(records: [DictionaryExportRecord]) throws -> Data {
    ...
    static func makeMergedPhrasesDatabase(phrases: [PhraseItem]) throws -> Data {
        let url = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent(UUID().uuidString)
            .appendingPathExtension("db")
        defer { try? FileManager.default.removeItem(at: url) }
