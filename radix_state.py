...
DEFAULT_STATE = {
    ...
    "backup_workspace_mode": False,
    "browse_content_mode": "Dictionary",
    "browse_page_sort_order": "Viewed",
    "browse_page_script_mode": "simplified",
    "selected_browse_collection_id": "",
...
class StateManager:
    ...
    def get_phrase_favourites(self) -> List[str]:
        return self.state.get("favourite_phrases_list", [])
    
    def get_history(self) -> List[str]:
        return self.state.get("history", [])

    def get_remembered(self) -> List[str]:
        return self.state.get("remembered_list", [])
    ...
    def toggle_phrase_favourite(self, word: str):
        word = (word or "").strip()
        if not word:
            return
        favs = [p for p in self.get_phrase_favourites() if isinstance(p, str)]
        if word in favs:
            favs.remove(word)
        else:
            favs.append(word)
        self.set("favourite_phrases_list", favs)

    def remember_character(self, char: str):
        """Add a character or phrase to the lightweight memory strip."""
        validated = self._normalize_memory_item(char)
        if not validated:
            return
        remembered = [c for c in self.get_remembered() if c != validated]
        remembered.insert(0, validated)
        self.set("remembered_list", remembered[:MEMORY_STRIP_LIMIT])

    def remove_from_remembered(self, char: str):
        remembered = [c for c in self.get_remembered() if c != char]
        self.set("remembered_list", remembered)

    def clear_remembered(self):
        self.set("remembered_list", [])

    def _normalize_memory_item(self, value: Any) -> str:
        ...
        if not item:
            return ""
        if len(item) == 1:
            return InputValidator.validate_character_input(item) or ""
        return item[:16]
    ...
    def clear_derived_widget_state(self):
        ...
        keys_to_clear = [k for k in list(self.state.keys()) if (
            ...
            k.startswith("fav_chk_") or k.startswith("memory_remove_")
        )]
...
class ConfigManager:
    ...
    def build_profile_dict(self) -> Dict:
        ...
        return {
            ...
            "favourite_phrases_list": self.state.get_phrase_favourites(),
            "remembered_list": self.state.get_remembered(),
    ...
    def import_profile_dict(self, data: Dict):
        ...
        self.state.set("favourites_list", self._validated_char_list(data.get("favourites_list", [])))
        self.state.set("favourite_phrases_list", self._validated_phrase_list(data.get("favourite_phrases_list", [])))
        self.state.set("remembered_list", self._validated_memory_list(data.get("remembered_list", []), limit=MEMORY_STRIP_LIMIT))
    ...
    def _validated_char_list(self, values: Any, limit: int | None = None) -> List[str]:
        ...
        for value in values:
            if not isinstance(value, str):
                continue
            validated = InputValidator.validate_character_input(value)
    ...
    def _validated_memory_list(self, values: Any, limit: int | None = None) -> List[str]:
        if not isinstance(values, list):
            return []
        cleaned = []
        seen = set()
        for value in values:
            normalized = self._normalize_memory_item(value)
    ...
    def _validated_phrase_list(self, values: Any, limit: int | None = None) -> List[str]:
        if not isinstance(values, list):
            return []
        cleaned = []
        seen = set()
