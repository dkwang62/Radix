...
struct SwiftUICrashGuardrailTests {
    ...
    func savedPageWorkspaceSwitcherIsShared() throws {
        ...
        let titles = try sourceText(at: "App/RootViewSupport.swift")
        let detailPane = try sourceText(at: "App/RootDetailPane.swift")
        ...
        #expect(switcher.contains("dateFormat = \"dd MMM yy\""))
        #expect(switcher.contains("let dateMode: PageCollectionSortOrder"))
        #expect(switcher.contains("collection.lastViewedAt ?? collection.createdAt"))
        #expect(switcher.contains("var labelTitle = \"Switch\""))
        #expect(switcher.contains("title: labelTitle"))
        #expect(!switcher.contains("title: \"Switch Page\""))
        #expect(switcher.contains("Section(\"Sort Pages\")"))
        #expect(browse.contains("PageWorkspaceSwitcher(selectedMode: .browse)"))
        #expect(browse.contains("SavedPageWorkspaceHeader("))
        #expect(browse.contains("browsePageSelectionSwitcher(collection)"))
        #expect(browse.contains("dateMode: browsePageSortOrder"))
        #expect(browse.contains("} scriptToggle: {\n                BrowseImageScriptToggle"))
        #expect(browse.contains("browsePageActionsMenu(collection, usesCompactLabel: true)"))
        ...
        #expect(browse.contains("get: { browsePageSortOrder }"))
        ...
        #expect(study.contains("studyPageSelectionSwitcher(\n                rowData.collection,"))
        #expect(study.contains("dateMode: studyPageSortOrder"))
        #expect(study.contains("} scriptToggle: {\n            studyScriptToggle"))
        #expect(study.contains("studySavedPageActionsMenu(collection, usesCompactLabel: true)"))
        #expect(study.contains("labelTitle: collectionDisplayName(collection)"))
        ...
        #expect(study.contains("SavedPageWorkspaceHeader("))
        ...
        #expect(titles.contains("map { _ in \"Browse - Pages\" }"))
        #expect(!titles.contains(#"Browse - \($0.name)"#))
        #expect(detailPane.contains("sidebar\n                .navigationTitle(\"\")"))

        let filter = try sourceText(at: "App/FilterGridTab.swift")
        #expect(filter.contains(".onChange(of: browsePageSortOrder)"))
        #expect(filter.contains("RadixBrowsePreferences.pageSortOrder = newValue"))
    }
