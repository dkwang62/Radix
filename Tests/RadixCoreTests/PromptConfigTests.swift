...
struct PromptConfigTests {
    ...
    func practiceGeneratorTaskAvailability() {
        ...
        #expect(generator?.template.contains("Every zh value must be no more than 16 Chinese characters") == true)
        #expect(generator?.template.contains("Skip any idea that cannot be expressed as useful complete practice material within the 16-character limit.") == true)
    ...
    func pageSentenceExtractorTaskAvailability() {
        ...
        #expect(extractor?.template.contains("Every zh value must be no more than 16 Chinese characters") == true)
        #expect(extractor?.template.contains("If a source idea cannot be turned into useful complete practice sentences of 16 Chinese characters or fewer, skip it.") == true)
    ...
    func practiceGeneratorRendersTopicContext() {
        ...
        #expect(prompt.contains("Every zh value must be no more than 16 Chinese characters"))
        #expect(prompt.contains("Skip any idea that cannot be expressed as useful complete practice material within the 16-character limit."))
    ...
    func pageSentenceExtractorRendersSavedPageContext() {
        ...
        #expect(prompt.contains("Every zh value must be no more than 16 Chinese characters"))
        #expect(prompt.contains("If a source idea is longer than 16 Chinese characters, reword it or split it"))
